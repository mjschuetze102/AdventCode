    ...
    public static void main(String[] args) throws IOException {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            for(String line; (line = reader.readLine()) != null; ) {
                ...
                if (
                    (elves.get(0).getKey() <= elves.get(1).getKey() && elves.get(0).getValue() >= elves.get(1).getKey()) ||
                    (elves.get(0).getKey() >= elves.get(1).getKey() && elves.get(0).getKey() <= elves.get(1).getValue())
                )   count++;
